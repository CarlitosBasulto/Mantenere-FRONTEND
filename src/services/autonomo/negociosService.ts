...
export const getNegocios = async () => {
    const res = await api.get(`/autonomo/negocios`);
    return fixUrls(res.data);
};

// Obtener un negocio por ID
export const getNegocio = async (id: number) => {
    const res = await api.get(`/autonomo/negocios/${id}`);
    return fixUrls(res.data);
};

// Crear un nuevo negocio
export const createNegocio = async (data: any) => {
    const res = await api.post(`/autonomo/negocios`, data);
    return res.data;
};

// Actualizar un negocio
export const updateNegocio = async (id: number, data: any) => {
    const res = await api.put(`/autonomo/negocios/${id}`, data);
    return res.data;
};
...
export const asignarEncargado = async (id: number, data: { name: string; email: string; password?: string }) => {
    const res = await api.post(`/autonomo/negocios/${id}/encargado`, data);
    return res.data;
};

// Obtener encargado (subgerente) del negocio
export const getEncargado = async (id: number) => {
    const res = await api.get(`/autonomo/negocios/${id}/encargado`);
    return res.data;
};
