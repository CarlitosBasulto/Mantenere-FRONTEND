...
export const getTrabajos = async (params?: { negocio_id?: number; trabajador_id?: number }) => {
    const res = await api.get(`/autonomo/trabajos`, { params });
    return fixUrls(res.data);
};

// Obtener un trabajo por ID
export const getTrabajo = async (id: number) => {
    const res = await api.get(`/autonomo/trabajos/${id}`);
    return fixUrls(res.data);
};

// Crear un nuevo trabajo
export const createTrabajo = async (data: any) => {
    const res = await api.post(`/autonomo/trabajos`, data);
    return res.data;
};
...
export const updateEstadoTrabajo = async (id: number, data: { estado: string; visitado?: boolean; hora_llegada?: string; latitud_llegada?: string; longitud_llegada?: string }) => {
    const res = await api.put(`/autonomo/trabajos/${id}/estado`, data);
    return res.data;
};

export const assignTrabajador = async (trabajoId: number, trabajadorId: number) => {
    const res = await api.put(`/autonomo/trabajos/${trabajoId}/asignar`, { trabajador_id: trabajadorId });
    return res.data;
};

// Actualizar un trabajo
export const updateTrabajo = async (id: number, data: any) => {
    const res = await api.put(`/autonomo/trabajos/${id}`, data);
    return res.data;
};

// Actualizar un trabajo (PATCH para actualizaciones parciales)
export const updateTrabajoPatch = async (id: number, data: any) => {
    const res = await api.patch(`/autonomo/trabajos/${id}`, data);
    return res.data;
};
// Eliminar un trabajo
export const deleteTrabajo = async (id: number) => {
    const res = await api.delete(`/autonomo/trabajos/${id}`);
    return res.data;
};
