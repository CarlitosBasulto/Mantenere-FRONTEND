...
export const getAdminAutonomos = async (): Promise<AdminAutonomo[]> => {
    const res = await api.get("/base/propietarios-autonomos");
    return res.data;
};
...
export const getAdminAutonomoDashboard = async (id: number): Promise<AdminAutonomoDashboard> => {
    const res = await api.get(`/base/propietarios-autonomos/${id}/dashboard`);
    return res.data;
};

// ── Sus negocios ───────────────────────────────────────────────────────────
export const getAdminAutonomoNegocios = async (id: number) => {
    const res = await api.get(`/base/propietarios-autonomos/${id}/negocios`);
    return res.data;
};

// ── Sus técnicos ───────────────────────────────────────────────────────────
export const getAdminAutonomoTrabajadores = async (id: number) => {
    const res = await api.get(`/base/propietarios-autonomos/${id}/trabajadores`);
    return res.data;
};

// ── Sus trabajos ───────────────────────────────────────────────────────────
export const getAdminAutonomoTrabajos = async (id: number) => {
    const res = await api.get(`/base/propietarios-autonomos/${id}/trabajos`);
    return res.data;
};

// ── Sus cotizaciones ───────────────────────────────────────────────────────
export const getAdminAutonomoCotizaciones = async (id: number) => {
    const res = await api.get(`/base/propietarios-autonomos/${id}/cotizaciones`);
    return res.data;
};

// ── Bloquear / Desbloquear ─────────────────────────────────────────────────
export const toggleBloqueoAdminAutonomo = async (id: number) => {
    const res = await api.put(`/base/propietarios-autonomos/${id}/bloquear`);
    return res.data;
};

// ── Gerente General ───────────────────────────────────────────────────────
export const getGerenteGeneral = async () => {
    const res = await api.get('/autonomo/gerente');
    return res.data;
};

export const asignarGerenteGeneral = async (data: { name: string; email: string; password?: string }) => {
    const res = await api.post('/autonomo/gerente', data);
    return res.data;
};
